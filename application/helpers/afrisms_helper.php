...
function sms_send($service_no, $sms_alert_data){
  $ci = & get_instance();
...
{ 
  ...
  $results = $gateway->sendMessage($recipients, $sms_alert_data['SMSMessage'], $from);
  
  foreach($results as $result) {
  $sms_result  = array('statusCode' => $result->statusCode,
                    'status' => $result->status,
                   'cost' =>  $result->cost,
                   'messageid' => $result->messageId,
                   'number' =>$result->number
                    );
 $message_log = json_encode($sms_result).PHP_EOL."-------------------------".PHP_EOL;
   file_put_contents('C:\xampp\htdocs\alertsender\log\log_'.date("j.n.Y").'.log', $message_log, FILE_APPEND);
   echo $message_log;

  $ci->Alertsend->update_sms_picked($service_no, $sms_alert_data['RowID'], $sms_result['status'], $sms_result['messageid'], $smsprocessed='E', $smsprocesseddate = date('Y-m-d H:i:s'));
 }
